          ...
          {isDigital && (
            <Stack spacing={2}>
              ...
              <CompactAccountAutocomplete
                currencyId={
                  selectedService?.Currency?.id ?? selectedService?.['Currency.id'] ?? selectedService?.currencyId ?? ''
                }
                value={selectedCompactAccount}
                onChange={(acc) => {
                  const a = acc || null;
                  setSelectedCompactAccount(a);
                  setPayAccountId(a?.id ? String(a.id) : '');
                  setPayAccount(a?.accountNumber ?? '');
                  setPayHolderName(a?.holderName ?? '');
                  setPayCurrencyId(a?.['Currency.id'] ? String(a['Currency.id']) : '');
                  setPayBankId(a?.['Bank.id'] ? String(a['Bank.id']) : '');
                  setPayCountryId(a?.['Bank.Country.id'] ? String(a['Bank.Country.id']) : '');
                }}
              />
