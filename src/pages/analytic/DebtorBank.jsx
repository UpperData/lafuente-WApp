...
const DebtorBank = () => {
  ...
  const handleExportExcel = async () => {
    try {
      ...
      // try to use SheetJS (xlsx) to generate a true .xlsx file
      const XLSXModule = await import('xlsx');
      const XLSX = XLSXModule?.default ?? XLSXModule;

      // build worksheet rows (AOA)
      const rows = [];
      rows.push(['La Fuente · Reporte Cuentas Deudoras']);
      rows.push([`Generado: ${new Date().toLocaleString()}`]);
      if (!isAll) {
        rows.push([`Titular: ${titular}`, `Cuenta: ${cuenta}`, `Banco: ${banco}`, `Moneda: ${moneda}`]);
      } else {
        rows.push(['Titular: Todos', 'Cuentas: Todas', 'Moneda: Todas']);
      }
      rows.push([]); // empty spacer
      // headers
      rows.push(headers);
      // data rows (already arrays)
      ...
      const dateSuffix = new Date().toISOString().slice(0, 10);
      const fileSuffix = !isAll ? `_${sanitizeFilename(titular)}_${sanitizeFilename(cuenta)}` : '';
      a.download = `debtor-banks_${dateSuffix}${fileSuffix}.xlsx`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      URL.revokeObjectURL(url);
    } catch (e) {
      console.error('Export Excel (xlsx) failed', e);
      // fallback: keep previous HTML .xls export if xlsx creation fails
      try {
        const { headers, data } = buildExportRows();
        const logoData = await fetchLogoDataUrl();
      ...
      {loading ? (
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} md={4}>
            {/* scrollable container for account cards (compact left column) */}
